...
إعادة بناء كاملة لـ iBrowser كمشروع مفتوح المصدر متكامل الإعدادات ومتعدد
مزودي الذكاء الاصطناعي.

## ميزات جديدة
- لوحة ذكاء اصطناعي جانبية تدعم ChatGPT وClaude وGemini وCopilot
  وPerplexity وDeepSeek وGrok وPoe، مع تصفح الصفحة واللوحة جنبًا إلى جنب
- زر "نسخ التحديد" لإرسال نص محدد من الصفحة مباشرة إلى محادثة الذكاء الاصطناعي
- إعدادات متكاملة داخل التطبيق: عام، المظهر، الخصوصية، الذكاء الاصطناعي،
  الإضافات، حول iBrowser
- نظام إضافات/موديولات مفتوح المصدر (`PLUGINS.md`) مع إضافة تجريبية
  "ملاحظات سريعة"
- سمة فاتحة إلى جانب السمة الداكنة الحالية، قابلة للتبديل من الإعدادات
- محرك بحث افتراضي قابل للتخصيص (Google, Bing, DuckDuckGo, Yandex)
- صفحة رئيسية قابلة للتخصيص

## تحسينات
- استبدال نظام التحديثات الوهمي (نتائج عشوائية) بتحقق حقيقي من GitHub Releases
- توحيد وضع توفير البيانات عبر مخزن إعدادات واحد بدل تكرار في `localStorage`
- دمج جميع قواعد ترويسات الشبكة (User-Agent، OAuth، Do Not Track) في معالج
  واحد بدل معالجين متضاربين كان ثانيهما يُلغي الأول فعليًا

## إصلاحات
- إصلاح خلل كان يمنع تحديث عنوان/رابط التبويب عند كل تنقل بسبب الإشارة إلى
  متغيرات غير معرّفة (`mainNav`, `pmsLink`, ...) متبقية من قالب قديم
- إزالة قياسات سرعة الإنترنت الوهمية من شريط الحالة، واستبدالها بحالة اتصال
  حقيقية مبنية على `navigator.onLine`
- روابط "حول البرنامج" الخارجية أصبحت تُفتح فعليًا في المتصفح الافتراضي
